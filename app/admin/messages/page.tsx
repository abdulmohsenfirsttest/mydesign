...
type Tab = "meetings" | "milestones" | "spaces" | "proposal" | "files" | "quotes";
type HubFile = { id: string; name: string; size: string | null; url: string; created_at: string };
...
export default function AdminProjectHub() {
  ...
  // Files (project-scoped uploads → shared to the client's portal)
  ...
  function fetchFiles() {
    supabase.from("files").select("*").eq("project_id", selectedId!).order("created_at", { ascending: false })
      .then(({ data }) => setFiles((data as HubFile[]) ?? []));
  }
  ...
  useEffect(() => {
    ...
    const nc = supabase.channel(`hub-notes-${selectedId}`)
      ...
      .subscribe();
    const fc = supabase.channel(`hub-files-${selectedId}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "files", filter: `project_id=eq.${selectedId}` }, fetchFiles)
      .subscribe();
  ...
  async function handleHubFilesUpload(fileList: FileList | null) {
    if (!fileList || !selectedId) return;
    ...
    if (failed.length) setFilesError(`Couldn't save ${failed.length} file(s): ${failed.join("; ")}`);
    fetchFiles();
    setFilesUploading(false);
  }

  async function deleteHubFile(id: string, url: string) {
    if (!confirm("Delete this file?")) return;
    setDeletingFile(id);
    const marker = "/storage/v1/object/public/files/";
    const idx = url.indexOf(marker);
    if (idx !== -1) await supabase.storage.from("files").remove([decodeURIComponent(url.slice(idx + marker.length))]);
  ...
  return (
    <div className="flex h-screen">
      {/* Project Sidebar */}
      <div className="w-64 border-r border-soft flex flex-col flex-shrink-0">
        <div className="px-5 py-4 border-b border-soft">
          <h1 className="text-foreground text-sm tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>PROJECTS</h1>
          {deleteError && <p className="text-red-400/70 text-xs mt-1" style={{ fontFamily: "var(--font-inter)" }}>{deleteError}</p>}
        </div>
        <div className="flex-1 overflow-y-auto">
          {projects.length === 0 ? (
            <p className="px-5 py-4 text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>No projects yet.</p>
          ) : projects.map(p => (
            <div key={p.id} className={`flex items-center border-b border-soft hover:bg-fill transition-colors ${selectedId === p.id ? "bg-fill" : ""}`}>
              <button onClick={() => { setSelectedId(p.id); setMeetings([]); setMilestones([]); setSpaces([]); setQuotes([]); setInternalQuote(null); setProposal(null); setShowMeetingForm(false); setShowMilestoneForm(false); setShowSpaceForm(false); setShowQuoteForm(false); setMilestoneError(""); setSpaceError(""); setPricingError(""); setProposalError(""); setMilestoneUploadError({}); }}
                ...
                <p className="text-muted-1 text-xs mb-0.5" style={{ fontFamily: "var(--font-inter)" }}>{p.client_name}</p>
                <p className="text-muted-2 text-xs truncate" style={{ fontFamily: "var(--font-inter)" }}>{p.name}</p>
              </button>
      ...
      <div className="flex-1 overflow-y-auto">
        {selected ? (
          <div className="max-w-2xl p-8">
            ...
            {/* Tabs */}
            <div className="flex border-b border-soft mb-8">
              {(["meetings", "milestones", "spaces", "proposal", "files"] as Tab[]).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`px-5 py-3 text-xs tracking-widest transition-colors border-b-2 -mb-px ${activeTab === tab ? "border-foreground text-foreground" : "border-transparent text-muted-2 hover:text-muted-1"}`}
                  style={{ fontFamily: "var(--font-inter)" }}>
                  {tab === "files" ? "UPLOAD FILES" : tab.toUpperCase()}
                </button>
              ))}
            </div>
              ...
              <>
                <div className="flex justify-end mb-6">
                  <button onClick={() => setShowMeetingForm(!showMeetingForm)}
                    className="px-5 py-2.5 border border-foreground text-foreground text-xs tracking-widest hover:bg-foreground hover:text-background transition-colors"
                    style={{ fontFamily: "var(--font-inter)" }}>+ Log Meeting</button>
                </div>

                {showMeetingForm && (
                  <form onSubmit={handleAddMeeting} className="border border-border bg-surface p-6 mb-8">
                    <h3 className="text-foreground text-sm tracking-widest mb-5" style={{ fontFamily: "var(--font-inter)" }}>NEW MEETING LOG</h3>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div className="col-span-2">
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Meeting Title</label>
                      ...
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Date</label>
                        <input required type="date" value={meetingForm.date} onChange={e => setMeetingForm(f => ({ ...f, date: e.target.value }))}
                          className="w-full bg-transparent border border-border text-muted-1 text-xs px-3 py-2.5 focus:outline-none focus:border-strong"
                          style={{ fontFamily: "var(--font-inter)" }} />
                      </div>
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Time</label>
                        <input required type="time" value={meetingForm.time} onChange={e => setMeetingForm(f => ({ ...f, time: e.target.value }))}
                          className="w-full bg-transparent border border-border text-muted-1 text-xs px-3 py-2.5 focus:outline-none focus:border-strong"
                          style={{ fontFamily: "var(--font-inter)" }} />
                      </div>
                      <div className="col-span-2">
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Meeting Type</label>
                        <select value={meetingForm.type} onChange={e => setMeetingForm(f => ({ ...f, type: e.target.value }))}
                          className="w-full bg-surface-2 border border-border text-muted-1 text-xs px-3 py-2.5 focus:outline-none focus:border-strong"
                          style={{ fontFamily: "var(--font-inter)" }}>
                    ...
                    <div className="mb-4">
                      <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Summary</label>
                      <textarea required rows={4} value={meetingForm.summary} onChange={e => setMeetingForm(f => ({ ...f, summary: e.target.value }))}
                        placeholder="What was discussed..."
                        className="w-full bg-transparent border border-border text-muted-1 text-xs px-3 py-2.5 focus:outline-none focus:border-strong placeholder-muted-4 resize-none"
                        style={{ fontFamily: "var(--font-inter)" }} />
                    </div>
                    <div className="mb-4">
                      <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Decisions <span className="text-muted-2 normal-case tracking-normal">(one per line)</span></label>
                    ...
                    <div className="mb-5">
                      ...
                      <button type="button" onClick={() => fileRef.current?.click()}
                        className="text-xs border border-border text-muted-2 px-4 py-2 hover:border-border hover:text-muted-1 transition-colors"
                        style={{ fontFamily: "var(--font-inter)" }}>Attach files</button>
                      {attachedFiles.map((f, i) => (
                        <p key={i} className="text-muted-2 text-xs mt-1" style={{ fontFamily: "var(--font-inter)" }}>↳ {f.name}</p>
                      ))}
                    </div>
                ...
                {meetings.length === 0 ? (
                  <div className="border border-soft bg-surface p-10 text-center">
                    <p className="text-muted-2 text-sm" style={{ fontFamily: "var(--font-inter)" }}>No meetings logged yet.</p>
                  </div>
                ) : (
                    ...
                    {meetings.map((m, i) => (
                      <div key={m.id} className="flex gap-5">
                        <div className="flex flex-col items-center flex-shrink-0 w-5">
                          <div className={`w-3 h-3 rounded-full mt-1 border-2 ${m.status === "Approved" ? "bg-foreground border-foreground" : "border-strong bg-transparent"}`} />
                          {i < meetings.length - 1 && <div className="w-px flex-1 mt-2 bg-fill" style={{ minHeight: "32px" }} />}
                        </div>
                        <div className={`flex-1 mb-8 border ${m.status !== "Approved" ? "border-border" : "border-soft"} bg-surface p-6`}>
                          <div className="mb-3">
                            <h3 className="text-foreground text-sm mb-1" style={{ fontFamily: "var(--font-inter)" }}>{m.title}</h3>
                            <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{m.date} · {m.time} · {m.type}</p>
                          </div>
                          {m.summary && <p className="text-muted-2 text-xs leading-relaxed mb-4" style={{ fontFamily: "var(--font-inter)" }}>{m.summary}</p>}
                          {m.decisions?.length > 0 && (
                            <ul className="space-y-1.5 mb-4">
                              {m.decisions.map((d, j) => (
                                <li key={j} className="flex items-start gap-2 text-xs text-muted-1" style={{ fontFamily: "var(--font-inter)" }}>
                                  <span className="text-muted-2 flex-shrink-0">—</span>{d}
                                </li>
                              ))}
                            </ul>
                          )}
                          {m.files?.length > 0 && (
                            <div className="space-y-2 mb-4">
                              <p className="text-muted-2 text-xs tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>FILES</p>
                              {m.files.map((f, j) => (
                                <div key={j} className="flex items-center gap-2 py-2 px-3 border border-soft bg-fill">
                                  ...
                                  <span className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{f.size}</span>
                                  <a href={f.url} target="_blank" rel="noopener noreferrer" className="text-muted-2 hover:text-muted-1 transition-colors ml-1">
                                  ...
                                  <button onClick={() => downloadFile(f.url, f.name)} className="text-muted-2 hover:text-muted-1 transition-colors">
            ...
            {activeTab === "milestones" && (
              <>
                ...
                {showMilestoneForm && (
                  <form onSubmit={handleAddMilestone} className="border border-border bg-surface p-6 mb-8">
                    <h3 className="text-foreground text-sm tracking-widest mb-5" style={{ fontFamily: "var(--font-inter)" }}>NEW MILESTONE</h3>
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Description <span className="text-muted-2 normal-case tracking-normal">(optional)</span></label>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Start Date</label>
                          <input type="date" required value={milestoneForm.start_date} onChange={e => setMilestoneForm(f => ({ ...f, start_date: e.target.value }))}
                            className="w-full bg-transparent border border-border text-muted-1 text-xs px-3 py-2.5 focus:outline-none focus:border-strong"
                            style={{ fontFamily: "var(--font-inter)" }} />
                        </div>
                        <div>
                          <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>End Date</label>
                      ...
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Deliverable <span className="text-muted-2 normal-case tracking-normal">(optional)</span></label>
                        <input type="file"
                          onChange={e => {
                          ...
                          className="w-full text-xs text-muted-2 file:mr-3 file:py-2 file:px-3 file:border file:border-border file:bg-surface-2 file:text-muted-1 file:text-xs file:cursor-pointer hover:file:border-strong"
                          style={{ fontFamily: "var(--font-inter)" }} />
                        <p className="text-muted-2 text-[11px] mt-1.5" style={{ fontFamily: "var(--font-inter)" }}>Attach the deliverable (moodboard, render, PDF…) to create this milestone as <span className="text-muted-1">Completed</span> in one step — or leave it and add the file later.</p>
                      </div>
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Status</label>
                ...
                {milestones.length === 0 ? (
                  <div className="border border-soft bg-surface p-10 text-center">
                    <p className="text-muted-2 text-sm" style={{ fontFamily: "var(--font-inter)" }}>No milestones yet.</p>
                  </div>
                ) : (
                  <div className="space-y-0">
                    {milestones.map((m, i) => (
                      <div key={m.id} className="flex gap-5">
                        <div className="flex flex-col items-center flex-shrink-0 w-5">
                          ...
                          {i < milestones.length - 1 && <div className="w-px flex-1 mt-2 bg-fill" style={{ minHeight: "32px" }} />}
                        </div>
                        <div className="flex-1 mb-6 border border-soft bg-surface p-5">
                          {editingMilestoneId === m.id ? (
                            <div className="space-y-3">
                              <input value={editM.name} onChange={e => setEditM(f => ({ ...f, name: e.target.value }))} placeholder="Milestone name"
                                className="w-full bg-transparent border border-border text-muted-1 text-xs px-3 py-2 focus:outline-none focus:border-strong placeholder-muted-4" style={{ fontFamily: "var(--font-inter)" }} />
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                <input type="date" value={editM.start_date} onChange={e => setEditM(f => ({ ...f, start_date: e.target.value }))}
                                  className="w-full bg-transparent border border-border text-muted-1 text-xs px-3 py-2 focus:outline-none focus:border-strong" style={{ fontFamily: "var(--font-inter)" }} />
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => saveEditMilestone(m.id)} disabled={savingEditM || !editM.name.trim()}
                                  className="px-4 py-2 border border-foreground text-foreground text-xs tracking-widest hover:bg-foreground hover:text-background transition-colors disabled:opacity-30" style={{ fontFamily: "var(--font-inter)" }}>{savingEditM ? "Saving..." : "Save"}</button>
                                <button onClick={() => setEditingMilestoneId(null)}
                                  className="px-4 py-2 border border-border text-muted-2 text-xs hover:border-border transition-colors" style={{ fontFamily: "var(--font-inter)" }}>Cancel</button>
                              </div>
                            </div>
                          ) : (
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <div className="flex items-center gap-2 mb-0.5">
                                <p className={`text-sm ${m.status === "Skipped" ? "text-muted-2" : "text-muted-1"}`} style={{ fontFamily: "var(--font-inter)" }}>{m.name}</p>
                                ...
                                {m.status === "Skipped" && (
                                  <span className="text-[10px] tracking-widest text-muted-2 border border-soft px-1.5 py-0.5" style={{ fontFamily: "var(--font-inter)" }}>Skipped</span>
                                )}
                              </div>
                              {m.description && <p className="text-muted-2 text-xs mb-1" style={{ fontFamily: "var(--font-inter)" }}>{m.description}</p>}
                              {(m.start_date && m.end_date)
                                ? <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{fmtDate(m.start_date)} – {fmtDate(m.end_date)}</p>
                                : m.due_date ? <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{fmtDate(m.due_date)}</p> : null}
                            </div>
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              <button onClick={() => moveMilestone(i, -1)} disabled={i === 0} className="text-muted-2 hover:text-muted-1 transition-colors disabled:opacity-20 p-1" title="Move up">
                              ...
                              <button onClick={() => moveMilestone(i, 1)} disabled={i === milestones.length - 1} className="text-muted-2 hover:text-muted-1 transition-colors disabled:opacity-20 p-1" title="Move down">
                              ...
                              <button onClick={() => openEditMilestone(m)} className="text-muted-2 hover:text-muted-1 transition-colors p-1" title="Edit">
                              ...
                              <select value={m.status} onChange={e => updateMilestoneStatus(m.id, e.target.value)}
                                ...
                                className={`bg-transparent border text-xs px-2 py-1 focus:outline-none cursor-pointer ${m.status === "Completed" ? "border-border text-muted-1" : m.status === "In Progress" ? "border-amber-400/30 text-amber-400/60" : "border-soft text-muted-2"}`}
                                style={{ fontFamily: "var(--font-inter)" }}>
                                {milestoneStatuses.map(s => <option key={s} disabled={s === "Completed" && (m.files ?? []).length === 0} style={{ background: "var(--surface)" }}>{s}</option>)}
                              </select>
                          ...
                          <div className="mt-4 pt-4 border-t border-soft">
                            <p className="text-muted-2 text-xs tracking-widest mb-2" style={{ fontFamily: "var(--font-inter)" }}>DELIVERABLES</p>
                            {(m.files ?? []).length > 0 ? (
                              <div className="space-y-2 mb-3">
                                {(m.files ?? []).map((f, j) => (
                                  <div key={j} className="flex items-center gap-2 py-2 px-3 border border-soft bg-fill">
                                    ...
                                    <button onClick={() => downloadFile(f.url, f.name, "files")} className="text-muted-2 hover:text-muted-1 transition-colors">
                                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                    </button>
                                  </div>
                                ))}
                              ...
                              <p className="text-muted-2 text-xs mb-3" style={{ fontFamily: "var(--font-inter)" }}>No deliverable attached yet — required before this milestone can be marked Completed.</p>
                            )}
                            <label className="inline-block text-xs border border-border text-muted-2 px-4 py-2 hover:border-border hover:text-muted-1 transition-colors cursor-pointer" style={{ fontFamily: "var(--font-inter)" }}>
            ...
            {activeTab === "spaces" && (
              <>
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <p className="text-muted-1 text-xs tracking-widest mb-1" style={{ fontFamily: "var(--font-inter)" }}>TOTAL AREA</p>
                    <p className="text-foreground text-lg tabular-nums" style={{ fontFamily: "var(--font-inter)" }}>{spacesTotal.toLocaleString("en-US")} <span className="text-muted-2 text-sm">sqm</span></p>
                  </div>
                ...
                {showSpaceForm && (
                  <form onSubmit={handleAddSpace} className="border border-border bg-surface p-6 mb-8">
                    <h3 className="text-foreground text-sm tracking-widest mb-5" style={{ fontFamily: "var(--font-inter)" }}>NEW SPACE</h3>
                    <div className="grid grid-cols-3 gap-4">
                      <div className="col-span-2">
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Space / Room</label>
                      ...
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Size (sqm)</label>
                ...
                {spaces.length === 0 ? (
                  <div className="border border-soft bg-surface p-10 text-center">
                    <p className="text-muted-2 text-sm" style={{ fontFamily: "var(--font-inter)" }}>No spaces recorded yet. Add the rooms / areas and their square meters.</p>
                  </div>
                ) : (
                  <div className="border border-soft bg-surface">
                    {spaces.map((s, i) => (
                      <div key={s.id} className={`flex items-center justify-between px-5 py-3.5 ${i < spaces.length - 1 ? "border-b border-soft" : ""}`}>
                        <span className="text-muted-1 text-sm" style={{ fontFamily: "var(--font-inter)" }}>{s.name}</span>
                        <div className="flex items-center gap-4">
                          <span className="text-muted-2 text-sm tabular-nums" style={{ fontFamily: "var(--font-inter)" }}>{Number(s.sqm).toLocaleString("en-US")} sqm</span>
                    ...
                    <div className="flex items-center justify-between px-5 py-3.5 border-t border-border">
                      <span className="text-muted-1 text-xs tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>TOTAL</span>
                      <span className="text-foreground text-sm tabular-nums" style={{ fontFamily: "var(--font-inter)" }}>{spacesTotal.toLocaleString("en-US")} sqm</span>
                    </div>
                  </div>
                )}

                {/* Request internal pricing from the Manager (staff-only price stays inside the hub). */}
                <div className="mt-6 border border-soft bg-surface p-6">
                  <p className="text-xs text-muted-2 mb-3 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>INTERNAL PRICING</p>
                  {!internalQuote ? (
                    <>
                      ...
                      {spaces.length === 0 && <p className="text-muted-2 text-xs mt-2" style={{ fontFamily: "var(--font-inter)" }}>Add at least one space first.</p>}
                    ...
                    <>
                      <p className="text-muted-1 text-sm" style={{ fontFamily: "var(--font-inter)" }}>Priced: SAR {Number(internalQuote.price_per_sqm).toLocaleString("en-US")}/sqm → SAR {Number(internalQuote.total).toLocaleString("en-US")} <span className="text-muted-2">({Number(internalQuote.sqm_total).toLocaleString("en-US")} sqm)</span></p>
                      ...
                      <button type="button" onClick={reRequestPricing} disabled={requestingPricing}
                        className="mt-3 px-4 py-2 border border-border text-muted-2 text-xs tracking-widest hover:border-strong hover:text-muted-1 transition-colors disabled:opacity-40"
            ...
            {activeTab === "proposal" && (
              <>
                ...
                <div className="border border-soft bg-surface p-5 mb-6">
                  <p className="text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>MANAGER PRICING</p>
                  {!internalQuote ? (
                    <p className="text-muted-1 text-xs" style={{ fontFamily: "var(--font-inter)" }}>No pricing requested yet. Request pricing from the Spaces tab first.</p>
                  ) : internalQuote.status === "approved" ? (
                    <p className="text-muted-1 text-sm" style={{ fontFamily: "var(--font-inter)" }}>Approved · SAR {Number(internalQuote.price_per_sqm).toLocaleString("en-US")}/sqm → SAR {Number(internalQuote.total).toLocaleString("en-US")}</p>
                  ) : (
                ...
                {proposal && (
                  <div className="border border-soft bg-surface p-5 mb-6">
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-muted-1 text-xs tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>PROPOSAL STATUS</p>
                      <div className="flex items-center gap-2">
                        {(proposal.status === "sent" || proposal.status === "approved") && (
                          <button type="button" onClick={reviseProposal}
                            className="text-xs border border-border text-muted-1 px-2.5 py-1 hover:border-strong hover:text-muted-1 transition-colors"
                            style={{ fontFamily: "var(--font-inter)" }}>Revise</button>
                        )}
                        <span className={`text-xs px-2.5 py-1 border ${proposal.status === "approved" ? "border-strong text-muted-1" : proposal.status === "rejected" ? "border-red-400/30 text-red-400/60" : proposal.status === "sent" ? "border-amber-400/30 text-amber-400/60" : "border-soft text-muted-2"}`}
                          style={{ fontFamily: "var(--font-inter)" }}>{proposal.status.toUpperCase()}</span>
                      </div>
                    </div>
                    {proposal.sent_at && <p className="text-muted-2 text-xs mt-2" style={{ fontFamily: "var(--font-inter)" }}>Sent {new Date(proposal.sent_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>}
                    {(proposal.status === "approved" || proposal.status === "rejected") && (
                      <div className="mt-3 pt-3 border-t border-soft">
                        {proposal.client_comment && (
                          <>
                            <p className="text-muted-2 text-xs tracking-widest mb-1" style={{ fontFamily: "var(--font-inter)" }}>CLIENT COMMENT</p>
                            <p className="text-muted-2 text-xs leading-relaxed mb-2" style={{ fontFamily: "var(--font-inter)" }}>{proposal.client_comment}</p>
                          </>
                        )}
                        {proposal.decided_at && <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>Decided {new Date(proposal.decided_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>}
                      </div>
                    )}
                  </div>
                )}
                ...
                {proposalEditable ? (
                  <form onSubmit={saveProposal} className="border border-soft bg-surface p-6">
                    <h3 className="text-foreground text-sm tracking-widest mb-5" style={{ fontFamily: "var(--font-inter)" }}>{proposal ? "EDIT PROPOSAL" : "NEW PROPOSAL"}</h3>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Scope of Work</label>
                      ...
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Stages of Work</label>
                      ...
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Pricing</label>
                      ...
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Terms &amp; Conditions</label>
                  ...
                  <div className="border border-soft bg-surface p-6 space-y-5">
                    <div>
                      <p className="text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>SCOPE OF WORK</p>
                      <p className="text-muted-1 text-xs leading-relaxed whitespace-pre-wrap" style={{ fontFamily: "var(--font-inter)" }}>{proposal?.scope || "—"}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>STAGES OF WORK</p>
                      <p className="text-muted-1 text-xs leading-relaxed whitespace-pre-wrap" style={{ fontFamily: "var(--font-inter)" }}>{proposal?.stages || "—"}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>PRICING</p>
                      <p className="text-muted-1 text-xs leading-relaxed whitespace-pre-wrap" style={{ fontFamily: "var(--font-inter)" }}>{proposal?.pricing || "—"}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>TERMS &amp; CONDITIONS</p>
                      <p className="text-muted-1 text-xs leading-relaxed whitespace-pre-wrap" style={{ fontFamily: "var(--font-inter)" }}>{proposal?.terms || "—"}</p>
                    </div>
                    <div className="pt-2 flex items-center gap-3">
                      {proposal?.pdf_url && (
                        <a href={proposal.pdf_url} target="_blank" rel="noopener noreferrer"
                          className="inline-block px-5 py-2.5 border border-foreground text-foreground text-xs tracking-widest hover:bg-foreground hover:text-background transition-colors"
                          style={{ fontFamily: "var(--font-inter)" }}>Download quotation (PDF)</a>
                      )}
                      <button onClick={regeneratePdf} disabled={generatingPdf}
                        className="inline-block px-5 py-2.5 border border-border text-muted-2 text-xs tracking-widest hover:border-strong hover:text-muted-1 transition-colors disabled:opacity-40"
                ...
                <div className="mt-8 border-t border-soft pt-6">
                  <h3 className="text-foreground text-sm tracking-widest mb-1" style={{ fontFamily: "var(--font-inter)" }}>INTERNAL NOTES</h3>
                  <p className="text-muted-2 text-xs mb-4" style={{ fontFamily: "var(--font-inter)" }}>Staff only — the client never sees these.</p>
                  <form onSubmit={handleAddNote} className="mb-4">
                    <textarea value={noteBody} onChange={e => setNoteBody(e.target.value)} rows={2}
                      placeholder="Add a private note for the team..."
                      className="w-full bg-transparent border border-border text-muted-1 text-xs px-3 py-2.5 focus:outline-none focus:border-strong placeholder-muted-4 resize-none mb-2"
                      style={{ fontFamily: "var(--font-inter)" }} />
                    ...
                    <button type="submit" disabled={savingNote || !noteBody.trim()}
                      className="px-5 py-2 border border-foreground text-foreground text-xs tracking-widest hover:bg-foreground hover:text-background transition-colors disabled:opacity-30"
                      style={{ fontFamily: "var(--font-inter)" }}>{savingNote ? "Adding..." : "Add note"}</button>
                  </form>
                  {notes.length === 0 ? (
                    <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>No notes yet.</p>
                  ) : (
                    <div className="space-y-2">
                      {notes.map(n => (
                        <div key={n.id} className="border border-soft bg-fill px-4 py-3">
                          <div className="flex items-start justify-between gap-3">
                            <p className="text-muted-1 text-xs leading-relaxed whitespace-pre-wrap flex-1" style={{ fontFamily: "var(--font-inter)" }}>{n.body}</p>
                          ...
                          <p className="text-muted-2 text-xs mt-1.5" style={{ fontFamily: "var(--font-inter)" }}>{n.author_name || "Staff"} · {new Date(n.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })} {new Date(n.created_at).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</p>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            {activeTab === "files" && (
              <div className="max-w-2xl">
                <div className="flex items-baseline gap-3 mb-1">
                  <h2 className="text-foreground text-sm tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>UPLOAD FILES</h2>
                  {files.length > 0 && <span className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{files.length} file{files.length === 1 ? "" : "s"}</span>}
                </div>
                <p className="text-muted-2 text-xs mb-6" style={{ fontFamily: "var(--font-inter)" }}>
                  Shared with {selected?.client_name || "the client"} in their portal for this project.
                </p>

                <input ref={hubFileRef} type="file" multiple className="hidden" onChange={e => handleHubFilesUpload(e.target.files)} />
                <div
                  onDragOver={e => { e.preventDefault(); setFilesDragging(true); }}
                  onDragLeave={() => setFilesDragging(false)}
                  onDrop={e => { e.preventDefault(); setFilesDragging(false); handleHubFilesUpload(e.dataTransfer.files); }}
                  onClick={() => hubFileRef.current?.click()}
                  className={`border-2 border-dashed p-10 text-center cursor-pointer transition-colors ${filesDragging ? "border-strong bg-fill" : "border-border hover:border-strong"}`}>
                  {filesUploading ? (
                    <p className="text-muted-1 text-sm" style={{ fontFamily: "var(--font-inter)" }}>Uploading…</p>
                  ) : (
                    <>
                      <svg className="w-8 h-8 text-muted-2 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                      <p className="text-muted-1 text-sm mb-1" style={{ fontFamily: "var(--font-inter)" }}>Drag &amp; drop or click to browse</p>
                      <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>PDF, JPG, PNG, DWG — any size</p>
                    </>
                  )}
                </div>
                {filesError && <p className="text-red-400/90 text-xs mt-3" style={{ fontFamily: "var(--font-inter)" }}>{filesError}</p>}

                {files.length === 0 ? (
                  <div className="mt-8 text-center border border-soft py-10 px-6">
                    <p className="text-muted-1 text-sm mb-1" style={{ fontFamily: "var(--font-inter)" }}>No files shared yet</p>
                    <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>Drawings, renders or documents you upload will appear here and in the client&apos;s portal.</p>
                  </div>
                ) : (
                    ...
                    {files.map(f => (
                      <div key={f.id} className="flex items-start justify-between py-3 border-b border-soft last:border-0 gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{f.size ?? ""}</p>
                        </div>
                        <div className="flex flex-col items-end gap-1 flex-shrink-0">
                          <div className="flex items-center gap-1">
                            <a href={f.url} target="_blank" rel="noopener noreferrer" className="p-2 -m-0.5 text-muted-2 hover:text-foreground transition-colors" title="Open" aria-label={`Open ${f.name}`}>
                            ...
                            <button onClick={() => deleteHubFile(f.id, f.url)} disabled={deletingFile === f.id} className="p-2 -m-0.5 text-red-400/60 hover:text-red-400 transition-colors disabled:opacity-30" title="Delete" aria-label={`Delete ${f.name}`}>
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                            </button>
                          </div>
                          {f.created_at && (
                            <span className="text-muted-3 text-[11px] whitespace-nowrap pr-1" style={{ fontFamily: "var(--font-inter)" }}>
                              Sent {new Date(f.created_at).toLocaleString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" })}
                            </span>
                          )}
                        </div>
                      </div>
            ...
            {activeTab === "quotes" && (
              <>
                ...
                {showQuoteForm && (
                  <form onSubmit={handleAddQuote} className="border border-border bg-surface p-6 mb-8">
                    <h3 className="text-foreground text-sm tracking-widest mb-5" style={{ fontFamily: "var(--font-inter)" }}>NEW QUOTE</h3>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Title</label>
                    ...
                    <div className="mb-4">
                      <label className="block text-xs text-muted-2 mb-2 tracking-widests" style={{ fontFamily: "var(--font-inter)" }}>Line Items</label>
                      <div className="space-y-2">
                        {quoteLines.map((line, idx) => (
                          <div key={idx} className="flex gap-2 items-center">
                            <input value={line.item} onChange={e => setQuoteLines(ls => ls.map((l, i) => i === idx ? { ...l, item: e.target.value } : l))}
                              placeholder="Item description"
                              className="flex-1 bg-transparent border border-border text-muted-1 text-xs px-3 py-2 focus:outline-none focus:border-strong placeholder-muted-4"
                              style={{ fontFamily: "var(--font-inter)" }} />
                            <input value={line.amount} onChange={e => setQuoteLines(ls => ls.map((l, i) => i === idx ? { ...l, amount: e.target.value } : l))}
                              placeholder="SAR 0"
                              className="w-28 bg-transparent border border-border text-muted-1 text-xs px-3 py-2 focus:outline-none focus:border-strong placeholder-muted-4"
                              style={{ fontFamily: "var(--font-inter)" }} />
                            {quoteLines.length > 1 && (
                              <button type="button" onClick={() => setQuoteLines(ls => ls.filter((_, i) => i !== idx))}
                                className="text-muted-2 hover:text-red-400/60 transition-colors text-lg leading-none px-1">×</button>
                            )}
                          </div>
                        ))}
                        <button type="button" onClick={() => setQuoteLines(ls => [...ls, { item: "", amount: "" }])}
                          className="text-xs text-muted-2 hover:text-muted-1 transition-colors mt-1"
                ...
                {quotes.length === 0 ? (
                  <div className="border border-soft bg-surface p-10 text-center">
                    <p className="text-muted-2 text-sm" style={{ fontFamily: "var(--font-inter)" }}>No quotes yet.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {quotes.map(q => (
                      <div key={q.id} className="border border-soft bg-surface p-5">
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div>
                            <p className="text-foreground text-sm mb-0.5" style={{ fontFamily: "var(--font-inter)" }}>{q.title}</p>
                            <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{new Date(q.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className={`text-xs px-2.5 py-1 border ${q.status === "Signed" ? "border-border text-muted-1" : q.status === "Pending Signature" ? "border-strong text-muted-1" : "border-soft text-muted-2"}`}
                              style={{ fontFamily: "var(--font-inter)" }}>{q.status}</span>
                            {q.status === "Signed" && (
                              <button onClick={() => unapproveQuote(q.id)}
                                className="text-xs border border-soft text-muted-2 px-2.5 py-1 hover:border-border hover:text-muted-1 transition-colors"
                        ...
                        <div className="space-y-1.5 mb-3">
                          {q.lines.map((l, j) => (
                            <div key={j} className="flex justify-between text-xs" style={{ fontFamily: "var(--font-inter)" }}>
                              <span className="text-muted-1">{l.item}</span>
                              <span className="text-muted-1">{l.amount}</span>
                            </div>
                          ))}
                          <div className="flex justify-between text-sm pt-2 border-t border-soft mt-2" style={{ fontFamily: "var(--font-inter)" }}>
                        ...
                        {q.files?.length > 0 && (
                          <div className="space-y-1.5">
                            {q.files.map((f, j) => (
                              <div key={j} className="flex items-center gap-2 py-1.5 px-3 border border-soft bg-fill">
                                <svg className="w-3 h-3 flex-shrink-0 text-muted-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                <span className="flex-1 text-muted-1 text-xs truncate" style={{ fontFamily: "var(--font-inter)" }}>{f.name}</span>
                                <span className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{f.size}</span>
                                <a href={f.url} target="_blank" rel="noopener noreferrer" className="text-muted-2 hover:text-muted-1 transition-colors ml-1">
