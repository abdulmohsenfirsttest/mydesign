...
const statusStyle: Record<string, string> = {
  "Pending Signature": "border-strong text-muted-1",
  "Signed": "border-border text-muted-2",
  "Draft": "border-soft text-muted-2",
};
  ...
  return (
    <div className="p-8">
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-3xl text-foreground mb-1" style={{ fontFamily: "var(--font-playfair)" }}>Quotes & E-sign</h1>
          <p className="text-muted-1 text-sm" style={{ fontFamily: "var(--font-inter)" }}>Create and send financial offers to clients.</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSend} className="border border-soft bg-surface p-6 mb-6">
          <h2 className="text-foreground text-sm tracking-widest mb-5" style={{ fontFamily: "var(--font-inter)" }}>NEW QUOTE</h2>
          <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            ...
            <div>
              <label className="block text-xs text-muted-2 mb-2 tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>Project</label>
          ...
          <h3 className="text-muted-2 text-xs tracking-widest mb-3" style={{ fontFamily: "var(--font-inter)" }}>LINE ITEMS</h3>
            ...
            {lines.map((line, i) => (
              <div key={i} className="flex items-center gap-3">
                <input required placeholder="Description" value={line.item}
                  ...
                  className="flex-1 bg-transparent border border-border text-muted-1 text-xs px-3 py-2.5 focus:outline-none focus:border-strong placeholder-muted-4" style={{ fontFamily: "var(--font-inter)" }} />
                <input required placeholder="SAR 0" value={line.amount}
                  onChange={e => { const l = [...lines]; l[i].amount = e.target.value; setLines(l); }}
                  className="w-32 bg-transparent border border-border text-muted-1 text-xs px-3 py-2.5 focus:outline-none focus:border-strong placeholder-muted-4" style={{ fontFamily: "var(--font-inter)" }} />
                {lines.length > 1 && (
          ...
          <button type="button" onClick={() => setLines(l => [...l, { item: "", amount: "" }])}
            className="text-muted-2 text-xs hover:text-muted-1 transition-colors mb-5" style={{ fontFamily: "var(--font-inter)" }}>+ Add line</button>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={saving}
              className="px-6 py-2.5 border border-foreground text-foreground text-xs tracking-widest hover:bg-foreground hover:text-background transition-colors disabled:opacity-40" style={{ fontFamily: "var(--font-inter)" }}>
              {saving ? "Saving..." : "Send for Signature"}
            </button>
            <button type="button" onClick={() => setShowForm(false)}
              className="px-6 py-2.5 border border-border text-muted-2 text-xs hover:border-border transition-colors" style={{ fontFamily: "var(--font-inter)" }}>Cancel</button>
          </div>
        </form>
      )}
          ...
          {quotes.map(q => (
            <div key={q.id} className="border border-soft bg-surface">
              <button onClick={() => setExpanded(expanded === q.id ? null : q.id)}
                className="w-full flex items-center justify-between px-6 py-5 hover:bg-fill transition-colors">
                <div className="flex items-center gap-6 text-left">
                  <div>
                    <p className="text-muted-1 text-sm mb-0.5" style={{ fontFamily: "var(--font-inter)" }}>{q.title || "—"} · {q.clients?.name ?? "—"}</p>
                    <p className="text-muted-2 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{q.projects?.name ?? "—"} · {new Date(q.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-foreground text-sm" style={{ fontFamily: "var(--font-playfair)" }}>SAR {total(q.lines)}</span>
                  ...
                  <svg className={`w-4 h-4 text-muted-2 transition-transform ${expanded === q.id ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 9l-7 7-7-7"/></svg>
                </div>
              </button>
              {expanded === q.id && (
                <div className="border-t border-soft px-6 py-5">
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="flex justify-between text-sm pt-3 border-t border-soft" style={{ fontFamily: "var(--font-inter)" }}>
                      ...
                      <span className="text-foreground">SAR {total(q.lines)}</span>
                    </div>
                  </div>
                  {q.files?.length > 0 && (
                    <div className="mt-4 space-y-2">
                      <p className="text-muted-2 text-xs tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>ATTACHMENTS</p>
                      {q.files.map((f, j) => (
