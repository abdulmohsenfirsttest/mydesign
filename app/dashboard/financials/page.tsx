...
const statusStyle: Record<string, string> = {
  ...
  "Draft": "border-soft text-muted-4",
};
...
export default function FinancialsPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl text-foreground mb-1" style={{ fontFamily: "var(--font-playfair)" }}>Financial Offers</h1>
        <p className="text-muted-2 text-sm" style={{ fontFamily: "var(--font-inter)" }}>Review and sign your project quotations.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="border border-soft bg-surface p-12 text-center">
          <p className="text-muted-4 text-sm" style={{ fontFamily: "var(--font-inter)" }}>No financial offers yet.</p>
        </div>
      ) : (
            ...
            <div key={q.id} className="border border-soft bg-surface">
              <button onClick={() => setExpanded(expanded === q.id ? null : q.id)}
                ...
                <div className="text-left">
                  <p className="text-foreground text-sm mb-0.5" style={{ fontFamily: "var(--font-inter)" }}>{q.title || q.projects?.name || "—"}</p>
                  <p className="text-muted-3 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{q.projects?.name ?? "—"} · {new Date(q.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <svg className={`w-4 h-4 text-muted-3 transition-transform ${expanded === q.id ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 9l-7 7-7-7"/></svg>
                </div>
              </button>
                ...
                <div className="border-t border-soft px-6 py-5">
                  ...
                  {q.files?.length > 0 && (
                    <div className="mb-5 space-y-2">
                      <p className="text-muted-4 text-xs tracking-widest" style={{ fontFamily: "var(--font-inter)" }}>ATTACHMENTS</p>
                      {q.files.map((f, j) => (
                        <div key={j} className="flex items-center gap-2 py-2 px-3 border border-soft bg-fill">
                          ...
                          <a href={f.url} target="_blank" rel="noopener noreferrer" className="text-muted-4 hover:text-muted-1 transition-colors ml-1" title="Open">
                          ...
                          <button onClick={() => { const sep = f.url.includes("?") ? "&" : "?"; const a = document.createElement("a"); a.href = `${f.url}${sep}download=${encodeURIComponent(f.name)}`; a.click(); }}
                            className="text-muted-4 hover:text-muted-1 transition-colors" title="Download">
