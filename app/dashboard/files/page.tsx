...
export default function FilesPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl text-foreground mb-1" style={{ fontFamily: "var(--font-playfair)" }}>Files & Deliverables</h1>
        <p className="text-muted-2 text-sm" style={{ fontFamily: "var(--font-inter)" }}>All documents and assets shared by the design team.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="border border-soft bg-surface p-12 text-center">
          <p className="text-muted-4 text-sm" style={{ fontFamily: "var(--font-inter)" }}>No files yet.</p>
        </div>
      ) : (
        <div className="border border-soft bg-surface">
          <div className="grid grid-cols-12 px-5 py-3 border-b border-soft text-muted-4 text-xs" style={{ fontFamily: "var(--font-inter)" }}>
          ...
          {files.map((f, i) => (
            <div key={f.id} className={`grid grid-cols-12 items-center px-5 py-4 hover:bg-fill transition-colors ${i < files.length - 1 ? "border-b border-soft" : ""}`}>
              <div className="col-span-5 flex items-center gap-3">
                <svg className="w-4 h-4 text-muted-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                <span className="text-muted-1 text-sm truncate" style={{ fontFamily: "var(--font-inter)" }}>{f.name}</span>
              </div>
              <span className="col-span-3 text-muted-3 text-xs truncate" style={{ fontFamily: "var(--font-inter)" }}>{f.projects?.name || "—"}</span>
              <span className="col-span-2 text-muted-3 text-xs" style={{ fontFamily: "var(--font-inter)" }}>{f.size}</span>
              <span className="col-span-1 text-muted-4 text-xs" style={{ fontFamily: "var(--font-inter)" }}>
