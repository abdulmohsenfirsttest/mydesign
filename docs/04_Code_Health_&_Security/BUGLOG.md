...
## BUG-015 — Nightly backups silently dead for 18 days (Drive mount moved) ✅ (2026-07-25)
- **Symptom:** no snapshot after `2026-07-05T23-00-05Z`. Nothing surfaced it: `launchctl list | grep mydesign` still showed the job loaded, and no one was checking `LATEST.txt` against the calendar.
- **Root cause:** the Google Drive desktop app moved its mount from `~/Google Drive/My Drive` to `~/Library/CloudStorage/GoogleDrive-<account>/My Drive`, leaving the old path as an **empty stub directory**. `scripts/run-backup.sh` and `scripts/backup-data.mjs` both hardcoded the old path, and `mkdir -p` recreated it locally — so the 2026-07-06 run wrote six tables into a folder Drive never synced, then crashed on `manifest.json` (`ENOENT`). After that the job stopped producing output at all.
- **Fix:** both scripts now probe `~/Library/CloudStorage/GoogleDrive-*/My Drive/mydesign` then `~/Google Drive/My Drive/mydesign`, and **abort with a logged error** instead of backing up to a path nobody reads. launchd agent reloaded; verified end-to-end (snapshot `2026-07-25T13-15-47Z`, `exit: 0`).
- **If it recurs:** check `LATEST.txt`'s timestamp against today **first** — "job is loaded" proves nothing. Then `tail ~/mydesign-backups/launchd.err.log` for `BACKUP ABORTED`, which means Drive is signed out/unsynced rather than a path bug.
- **Lesson:** a backup that fails silently is worse than no backup, because it buys false confidence. Any cloud-synced destination path is a moving target — resolve it at runtime and fail loudly.

## BUG-016 — Backups truncated to one table: `EDEADLK` writing to the Drive mount 🟠 OPEN (since 2026-08-02)
- **Symptom:** every snapshot from `2026-08-02` onward contains **only `admins.json`** — no `manifest.json`, no other 10 tables. `LATEST.txt` is frozen at **`2026-08-01T23-09-57Z`** (the last complete backup) and `backup.log` stopped gaining entries after 2026-08-02, so the log itself no longer reports the failure. `launchctl list | grep mydesign` shows **last exit code 1**.
- **Root cause (probable):** writes to the Google Drive FUSE mount intermittently fail with **`Resource deadlock avoided` (EDEADLK)**. `~/mydesign-backups/launchd.err.log` shows it on `run-backup.sh` lines 29/31/39 (the log `echo`s) and `git archive` dies the same way (`code snapshot: 128`). Two Drive processes are running at **different versions (127.0.1.0 and 129.0.1.0)** with `--auto_restart_count=2`, so the app self-updated around 2026-08-02 — the 02:00 backup lands while DriveFS is re-indexing. Writes succeed when tested by hand during the day, which is why this looks fine on a manual check. Disk is also at **93% (15 GiB free)**, which may aggravate it.
- **Why the BUG-015 fix didn't catch it:** that fix guarantees we write to the *right* folder, and it aborts if the folder is missing. It does **not** verify the write *succeeded* — `admins.json` (the first table alphabetically) goes through, the next write hits EDEADLK, and the process dies mid-snapshot leaving a directory that looks like a backup.
- **Proposed fix (not yet applied):** stop writing directly to the FUSE mount. Build each snapshot in a **local staging dir**, verify it (all expected tables present + `manifest.json` written), then copy into Drive with a retry; treat a partial snapshot as a **hard failure** that prunes the bad directory and logs to a path outside Drive. Also gate the source-zip `git archive` on the same retry.
- **If it recurs:** `ls` a few recent snapshot dirs and count files — 12 is healthy, 1 is this bug. Never trust `backup.log` alone here; when the mount is unwritable the log is the *first* thing that stops recording.
- **⚠️ Data exposure:** the only complete snapshot is 2026-08-01. Anything changed since then is unbacked-up.

## BUG-017 — A project's stage and progress could never change ✅ (v4.9.2)
- **Symptom:** reported by a designer (2026-08-16, video) on production. Balconia Tower Apartment had one milestone **Completed** (with a 20.8 MB deliverable), one **In Progress**, one **Skipped** — and the hub header still read **`Quotation` / `0%`**, as did the Projects list, the Admin Dashboard and the client's portal. Every project in the account was stuck the same way. Paraphrased: *"whatever changes they make in the project, it doesn't change in the milestones."*
- **Root cause:** `projects.stage` / `projects.progress` were written in exactly **two** places in the whole codebase — project creation (`app/admin/projects/page.tsx:76`, always `Quotation`/0) and the **client** approving a proposal (`app/dashboard/projects/[id]/page.tsx:126`, always `Mood Board`/28). The one function that can move a project through the seven stages, `updateStage()` at `app/admin/messages/page.tsx:740`, was **fully written and had zero callers** — its dropdown was removed in **v4.3.0** and never replaced, leaving the function, its `stageProgress` map and its `updatingStage` flag as dead code for six months. Nothing links milestones to the project: all six milestone writes touch only the `milestones` table, and there is **no DB trigger** in any of the nine migrations. So staff could complete every milestone and nothing would ever move — and no UI existed to set it by hand either.
- **Fix:** the hub header's read-only stage `<span>` is now a `<select>` over the seven stages wired to the existing `updateStage()`. `updateStage()` also now **checks its Supabase error** — it previously did a bare `await …update()` and updated local state regardless, so a rejected write would have displayed the new stage and silently not saved (the BUG-010 trap). Errors render under the badges.
- **If it recurs:** grep for callers before trusting a handler — `grep -rn "updateStage" app` should return the definition **and** the `onChange`. One hit means the control was removed again.
- **Lesson:** deleting a UI control silently orphans its handler. Lint only warns for unused *variables*, and `updateStage` was a function referenced by nothing, so nothing flagged it. When a feature is removed, remove its handler too — or the next person reads the code, sees a working stage-update path, and assumes the feature exists.
- **Still open (separate):** `progress` is not derived from milestone completion. Marking milestones Completed still moves nothing on its own; stage is set by hand. Milestone titles are free text, so they can't be mapped onto the seven fixed stages automatically — deriving the **percentage** from completed-vs-total is the tractable half.
