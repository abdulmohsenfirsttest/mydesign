...
- [ ] Commit the uncommitted v3.2.0 working set (19 files / ~1,800 lines + `supabase/admins.sql`) so the deployed surface is actually in git. The large deployed-but-unversioned surface is the #1 health risk and blocks any safe rollback.
...
- [ ] Enable RLS on **every** data table (bookings, clients, projects, milestones, meetings, quotes, files) with proper per-row policies so a client can only see their own rows and the anon key can write nothing it shouldn't. Test in a branch/test environment before flipping in production.
