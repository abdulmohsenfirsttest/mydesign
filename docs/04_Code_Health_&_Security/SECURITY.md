...
| 3 | **CRITICAL** | `admins` table has `using (true)` policies for **select AND (since 2026-07-19 / v4.8.2, migration `0009`) insert/update/delete**. | RLS is technically enabled but the anon key can now **read and WRITE** the credentials table — read owner/staff `name`/`phone`/**plaintext password**, and create/modify/delete any staff (incl. a `manager`) account. Escalated from read-only to full CRUD to unblock self-service staff management on the pre-launch build (owner decision). **Phase 2 must revert this**: move staff management to server-side (service-role), hash passwords, and drop the anon read/write on `admins`. |
