...
- **What:** A designer filmed production showing Balconia Tower Apartment with a **Completed** milestone (20.8 MB deliverable attached), an **In Progress** one and a **Skipped** one — while the hub header, the Projects list, the Admin Dashboard and the client's portal all still read **`Quotation` / `0%`**. Cause: `projects.stage`/`progress` were written in only two places — project creation and the client approving a proposal — and `updateStage()` at `app/admin/messages/page.tsx:740`, the one function that walks a project through the seven stages, **had zero callers** since v4.3.0 deleted its dropdown. Milestones never touch `projects` and there is no DB trigger, so completing work moved nothing and no manual control existed either. The header stage badge is now a `<select>` over the seven stages calling the existing `updateStage()`; that function now also **reads its Supabase error** rather than updating local state unconditionally, and errors render under the badges.
